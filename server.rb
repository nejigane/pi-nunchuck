...
EM.run do
  ...
  puts 'nunchuk id: ' + nunchuck.read(0x52, 6, 0xfa).unpack('H6').join('')

  EM::WebSocket.run(:host => '0.0.0.0', :port => 8080) do |ws|
    ws.onopen { |handshake|
      puts 'WebSocket connection open'
      ws.send "{path: '#{handshake.path}'}"
    }
    ws.onclose { puts 'Connection closed' }
    ws.onmessage { |msg|
      ws.send status.to_json if msg == 'status'
    }
  end
  ...
  EM.add_periodic_timer(0.03) do
    begin
      ...
      STDERR.puts 'Failed to fetch nunchuck data'
    end
  end
end
